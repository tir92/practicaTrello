...
const getLocalStorageColumns = () => localStorage.getItem('columns') ? JSON.parse(localStorage.getItem('columns')) : [];
...
columns.forEach(column => {
    ...
    <div class="column" id="${column.id}">
        <div class="titulo">
            <h2>${column.title}</h2>
             <i class="far fa-trash-alt" onclick="removeColumnId(${column.Id})"></i>
        </div>
...
const removeColumnFromLocalStorage = (columnId) => {
    ...
    const columnsFiltered = columns.filter(col => columnId !== col.id)
    columns = columnsFiltered;
    localStorage.setItem('columns', JSON.stringify(columns));
}
const removeColumnId = (columnId) => {
...
document.querySelector('.addColumn').onkeyup = event => {
    if (event.key === "Enter") {
        ...
        <div class="column" id="${columnId}">
            <div class="titulo">
                <h2>${title}</h2>
                <i class="far fa-trash-alt" onclick="removeColumnId(${columnId})"></i>
            </div>
